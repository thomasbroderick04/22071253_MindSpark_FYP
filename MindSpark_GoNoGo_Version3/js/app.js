...
(() => {
  ...
  function proceedToNextTrialOrEnd() {
  ...
  function fadeStimulusToBackground() {
    ...
    if (!stimulus) return;
    stimulus.style.background = "#FFFFFF";
    stimulus.style.boxShadow = "none";
  }
    ...
    if (method === "V") {
      ...
      ws.addEventListener("message", (evt) => {
        ...
        if (msg.type === "event" && msg.reason === "serial_changed") {
          ...
          // has been detected by the system by changing the 'Go' button's appearance.
          if (goBtn) {
            goBtn.style.background = "#014841";
            setTimeout(() => {
              goBtn.style.background = "#007E72";
            }, 200);
          }
  ...
  document.addEventListener("DOMContentLoaded", () => {
    ...
    switch (file) {
      ...
      case "gonogo-test-touch-circle.html": // These are 'No-Go' pages.
      case "gonogo-test-verbal-circle.html":
        onStimulusPage(true);
        break;

      case "gonogo-test-touch-square.html": // These are 'Go' pages.
      case "gonogo-test-verbal-square.html":
        onStimulusPage(false);
        break;

      case "gonogo-test-incorrect.html": 
      case "gonogo-test-correct.html": 
        onFeedbackPage();
        break;
